...
fitness_chinese <- function(dfj) {
	...
	tc <- table(dfc$tsuit)
	sc <- union(paste0(dfc$tsuit, dfc$lrank), paste0(dfc$tsuit, dfc$rrank))
...
fitness_dom6 <- function(dfj) {
	...
	s6 <- union(paste0(df6$tsuit, df6$lrank), paste0(df6$tsuit, df6$rrank))
	# length(tdom6) - sum(abs(tdom6 - 7)) + length(s6)

	# each French suit has two halves for each rank
	tdom6 <- table(c(paste0(df6$tsuit, df6$lrank), paste0(df6$tsuit, df6$rrank)))
...
evolve <- function(filename) {
	...
	while (f < F_1) {
		...
		if (as.numeric(difftime(Sys.time(), start_time), units = "mins") > 30) {
